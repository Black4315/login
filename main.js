...
let loginmood ;
   ...
   &&  dataaccount[i].pass === passwordInput.value.toLowerCase()){
    ...
    loginmood = 'success'
          ...
          if(loginmood === 'success' || loginmood === 2 + 'success' ){
            p.preventDefault()
           window.location.href = './success.html';
           setMessegeError(loginForm,"success","")
           inputError(emailInput,'success','')
           inputError(passwordInput,'success',"") ;
...
createAccountForm.addEventListener('submit',(p)=>{
  ...
  if(username.value !=='' && emailInputsign.value !=='' && passwordInputsign.value !=='' && comPasswordInputsign.value !=='' && moodUsername !== 'error' &&moodEmail !== 'error'&& comfirmPass === 'success'){
    ...
    localStorage.setItem('Accounts' , JSON.stringify(dataaccount));
      ...
      for (let i = 0; i < inputSucsessSign.length; i++) {
        ...
        inputError(inputSucsessSign[i],'success','')
        }
...
{
  inputError(emailInputsign,'success',"") ;
...
comPasswordInputsign.classList.remove('input-form-sucsess')
    ...
    inputError(comPasswordInputsign,'success','')
...
function cleardatasign(){
 ...
 passwordInputsign.value= ''
   ...
   inputError(emailInputsign,'success','')
   inputError(passwordInputsign,'success',"") ;
          }
